...
# Define the ICM model
class ICMModel(nn.Module):
    def __init__(self, state_size, action_size, feature_size=256):
        ...
        self.forward_model = nn.Sequential(
            nn.Linear(feature_size + action_size, feature_size),
            nn.ReLU(),
            nn.Linear(feature_size, feature_size)
        )
    ...
    def forward(self, state, next_state, action):
        ...
        phi_next_s = self.feature(next_state)

        # Inverse model prediction
        inverse_input = torch.cat((phi_s, phi_next_s), dim=1)
        pred_action = self.inverse(inverse_input)

        # Dynamics model prediction
        forward_input = torch.cat((phi_s, action), dim=1)
        pred_phi_next_s = self.forward_model(forward_input)

        return pred_action, pred_phi_next_s, phi_next_s
...
class CustomEnv(gym.Env):
    def __init__(self, num_fruits=1, show_gui=False):
        ...
        observation_length = 4 + 2 * num_fruits + 18
        ...
        # Initialize ICM
        self.state_size = observation_length
        self.action_size = self.action_space.n
        self.icm_model = ICMModel(self.state_size, self.action_size)
        self.icm_optimizer = optim.Adam(self.icm_model.parameters(), lr=0.001)
    ...
    def step(self, action):
        ...
        # ICM calculations
        state = torch.FloatTensor(self.get_observation()).unsqueeze(0)
        next_state, _ = self.get_observation(), {}
        next_state = torch.FloatTensor(next_state).unsqueeze(0)
        action_onehot = torch.zeros(self.action_size)
        action_onehot[action] = 1.0
        action_tensor = action_onehot.unsqueeze(0)

        pred_action, pred_phi_next_s, phi_next_s = self.icm_model(state, next_state, action_tensor)
        intrinsic_reward = torch.mean((phi_next_s - pred_phi_next_s) ** 2, dim=1).item()

        # Combine extrinsic and intrinsic rewards
        self.reward = reward + intrinsic_reward
    ...
    def calculate_rewards(self, terminated):
        ...
        if terminated:
            ...
            reward += 0.01  # Small reward for each action
    ...
    def random_fruit_position(self):
        return [random.randrange(1, ((window_x - 20) // 10)) * 10, random.randrange(1, ((window_y - 20) // 10)) * 10]
    ...
    def get_observation(self):
        ...
        closest_fruit = min(self.fruit_positions, key=lambda x: ((x[0] - head_x) ** 2 + (x[1] - head_y) ** 2) ** 0.5)
        fruit_direction = [
